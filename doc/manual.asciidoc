...
arrays, either stack based or allocated from a linear allocator. All
...
Almost all developers are using multi-core machines, so it is important that
...
concurrently. This is very hard to diagnose and work around, leading to various
hacks and voodoo surrounding the build process.
