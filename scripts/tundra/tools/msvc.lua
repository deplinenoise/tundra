...
function apply(env, options)
	...
	env:set_many {
		...
		["PCHCOMPILE"] = "$(CC) /c $(_CPPDEFS) $(CPPPATH:b:p/I) /nologo $(CCOPTS) $(CCOPTS_$(CURRENT_VARIANT:u)) /Yc$(_PCH_HEADER) /Fp$(@:b) /Fo$(_PCH_OBJECT_FILE:b) $(<:[1]:b)",
		["LIBS"] = "",
		["PROGOPTS"] = "",
		["PROGCOM"] = "$(LD) /nologo @RESPONSE|@|$(_USE_PDB_LINK) $(PROGOPTS) $(LIBPATH:b:p/LIBPATH\\:) $(_USE_MODDEF) $(_PCH_OBJECT_FILE:b) $(LIBS) /out:$(@:b) $(<:b)",
		["LIBOPTS"] = "",
		...
		["SHLIBPREFIX"] = "",
		["SHLIBOPTS"] = "",
		["SHLIBCOM"] = "$(LD) /DLL /nologo @RESPONSE|@|$(_USE_PDB_LINK) $(SHLIBOPTS) $(LIBPATH:b:p/LIBPATH\\:) $(_USE_MODDEF) $(_PCH_OBJECT_FILE:b) $(LIBS) /out:$(@:b) $(<:b)",
